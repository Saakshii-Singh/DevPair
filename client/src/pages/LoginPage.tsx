  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        localStorage.setItem('vantage_rating', data.user.rating.toString());
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-6 pt-16">
      <div className="max-w-md w-full bg-surface/30 p-8 rounded-3xl border border-hairline space-y-6 shadow-xl relative overflow-hidden">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-zinc-500 text-xs">Enter your details to access your assessment scorecard cockpit.</p>
        </div>
        ...
        <p className="text-[11px] text-zinc-500 text-center">
          New to Vantage? <Link to="/signup" className="text-accent font-semibold hover:underline">Create an account</Link>
        </p>
      </div>
    </div>
  );
